...
const handleDelete = async (id) => {
    console.log(id);
    await axios.delete(`http://localhost:5005/Offre/deleteOffer/${id}`)
      .then((res) => res.data)
  ...
  const handlesearch = async () => {
    await axios
      .get(
        `http://localhost:5005/Offre/getOfferNom/${search}`
      )
      .then((res) => res.data)
      .then((data) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "id",
      render: (_, record) => (
        <Link to={`modifier-offre/${record.id}`}>
          <FaEdit
            className="fs-4"
  ...
  return (
    ...
    />
     <Button
              ...
              >
             <Link to={"ajouter-offre"} style={{ textDecoration:"none" , color:"white"}}> Ajouter un offre </Link> 
              </Button>
      <Divider />
